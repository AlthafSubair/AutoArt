...
const EditProfile = ({ph, whatsapp, address, id}) => {
  ...
  const [error, setError] = useState(null);
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          ...
          const response = await axios.put(`http://localhost:3001/api/user/updatecontact/${id}`, formData)
  ...
  return (
    <div>
      ...
      <div className="mb-4">{error && <p className='text-red-700'>{error}</p>}</div>
        <h1 className='text-center p-2 m-2 text-xl font-black text-gray-700 uppercase tracking-wide'>EDIT Contact Details</h1>
